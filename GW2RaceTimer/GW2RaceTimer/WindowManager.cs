...
namespace GW2RaceTimer
{
    ...
    class WindowManager
    {
        ...
        public HWND Handle
        {
            get
            {
                if (handle == HWND.Zero && (ClassName != string.Empty || Caption != string.Empty))
                {
        ...
        public WindowManager(HWND handle)
        {
            this.handle = handle;
        }

        public WindowManager(string className, string caption)
        {
            ClassName = className;
            Caption = caption;
        }
        ...
        public RECT WindowRect
        {
            get
            {
                ...
                GetClientRect(Handle, out targetWindowRect);
        ...
        public bool ClientToScreen(ref POINT lpPoint)
        {
            return ClientToScreen(Handle, ref lpPoint);
        }
        ...
        [DllImport("user32.dll", SetLastError = true)]
        private static extern UInt32 GetWindowLong(HWND hWnd, int nIndex);

        public UInt32 GetWindowLong(int nIndex)
        {
            return GetWindowLong(Handle, nIndex);
        }

        [DllImport("user32.dll")]
        private static extern int SetWindowLong(HWND hWnd, int nIndex, IntPtr dwNewLong);

        public int SetWindowLong(int nIndex, IntPtr dwNewLong)
        {
            return SetWindowLong(Handle, nIndex, dwNewLong);
        }

        [DllImport("user32.dll")]
        private static extern bool SetLayeredWindowAttributes(HWND hWnd, uint crKey, byte bAlpha, uint dwFlags);
        public bool SetLayeredWindowAttributes(uint crKey, byte bAlpha, uint dwFlags)
        {
            return SetLayeredWindowAttributes(Handle, crKey, bAlpha, dwFlags);
        }
        ...
        public bool SetWindowPos(HWND hWndInsertAfter, int X, int Y, int cx, int cy, SetWindowPosFlags uFlags)
        {
        ...
        {
            if (nCode >= 0 && wParam == (IntPtr)WM.KEYDOWN)
            {
